...
cp /poc/natural/SPIKEWRT.NSP $FUSER/SPIKE/SRC/
...
export TERM=xterm
rm -f /poc/data/spike_s4_result.txt
set +e
...
./natural udb=1 madio=0 "etid=S$$" \
  "stack=(LOGON SPIKE;RUN SPIKEWRT;FIN)" \
  </dev/null >/tmp/spike-s4-screen.out 2>&1
rc=$?
echo "--- natural rc=$rc ---"
if [ ! -f /poc/data/spike_s4_result.txt ]; then
  echo "S4 FAILED: no result file produced. Screen output:"
  cat /tmp/spike-s4-screen.out
  exit 1
fi
echo "--- result file ---"
cat /poc/data/spike_s4_result.txt
